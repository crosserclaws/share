...
class FakeRepository(repository.AbstractRepository):
    ...
    def _get_by_batchref(self, batchref: str) -> typing.Optional[model.Product]:
...
class TestAddBatch:
    ...
    def test_for_existing_product(self):
        ...
        messagebus.handle(events.BatchCreated(
            "b1", "EXIST-MIRROR", 100, None), uow)
        messagebus.handle(events.BatchCreated(
            "b2", "EXIST-MIRROR", 100, None), uow)
...
class TestAllocate:
    def test_returns_allocation(self):
        uow = FakeUnitOfWork()
        messagebus.handle(events.BatchCreated(
            "b1", "COMPLICATED-LAMP", 100, None), uow)

        results = messagebus.handle(events.AllocationRequired(
...
class TestChangeBatchQuantity:
    def test_changes_available_quantity(self):
        uow = FakeUnitOfWork()
        messagebus.handle(events.BatchCreated(
            "b1", "CHANGE-AVA-QTY", 100, None), uow)
        [batch] = uow.products.get(sku="CHANGE-AVA-QTY").batches
        assert batch.available_quantity == 100

        messagebus.handle(events.BatchQuantityChanged(
            batch.reference, 50), uow)
